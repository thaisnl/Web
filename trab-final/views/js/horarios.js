...
$(document).ready(() => {
    ...
    $.ajax({
        type: "GET",
        url: serverURL + '/api/horarios?crm=' + urlParams.get('crm') +'&data=' + urlParams.get('data'),
...
let addCliqueDoBotao = function(horario) {
    const urlParams = new URLSearchParams(window.location.search);
    window.location.replace('consulta.html?crm=' + urlParams.get('crm') +'&data=' + urlParams.get('data') + '&horario=' + horario);
}
